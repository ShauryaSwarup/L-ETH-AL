...
describe("WorkerCompanyMgmt Test", function () {
	...
	it("Should Return Workers by Location", async function () {
		...
		const workersByLocation = await workerCompanyMgmt.getWorkersByLocation(
			"Vasant Vihar"
		);
	...
	// it("Should check if an address is a Worker", async function () {
	// 	const isWorker = await workerCompanyMgmt.isWorker(owner.address);
	// 	expect(isWorker).to.be.false;

	// 	await workerCompanyMgmt.addWorker(owner.address, "Vasant Vihar");
	// 	const isWorkerAfterAddition = await workerCompanyMgmt.isWorker(
	// 		owner.address
	// 	);
	// 	expect(isWorkerAfterAddition).to.be.true;
	// });

	// it("Should check if an address is a Company", async function () {
	// 	const isCompany = await workerCompanyMgmt.isCompany(owner.address);
	// 	expect(isCompany).to.be.false;

	// 	await workerCompanyMgmt.addCompany("Kalpataru", owner.address);
	// 	const isCompanyAfterAddition = await workerCompanyMgmt.isCompany(
	// 		owner.address
	// 	);
	// 	expect(isCompanyAfterAddition).to.be.true;
	// });

	// it("Should Return Jobs Posted by Company", async function () {
	// 	const companyName = "Kalpataru";
	// 	const walletAddress = ethers.utils.getAddress(
	// 		"0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266"
	// 	);
	// 	await workerCompanyMgmt.addCompany(companyName, walletAddress);
	...
	// 	const postedJobs = await workerCompanyMgmt.getJobsPostedByCompany(
	// 		walletAddress
	// 	);

	// 	expect(postedJobs).to.have.lengthOf(2);

	// 	expect(postedJobs[0].location).to.equal("Thane");
	// 	expect(postedJobs[0].salary).to.equal(ethers.utils.parseEther("0.01"));
	// 	expect(postedJobs[0].vacancies).to.equal(10);

	// 	expect(postedJobs[1].location).to.equal("Mumbai");
	// 	expect(postedJobs[1].salary).to.equal(ethers.utils.parseEther("0.02"));
	// 	expect(postedJobs[1].vacancies).to.equal(20);
	// });
});
